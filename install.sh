...
# ==========================================
# 🚀 VLESS Shop Bot - Professional Installer
# ==========================================
...
check_dependencies() {
    ...
    sudo apt-get update || {
        echo -e "${RED}apt-get update failed. Please check package repositories.${NC}"
        exit 1
    }
...
configure_environment() {
    ...
    if [ -f "$TARGET_ENV" ]; then
        ...
        # Interactive Setup
        read -s -p "Enter Telegram Bot Token: " BOT_TOKEN
        echo
...
setup_nginx_ssl() {
    ...
    if [ -d "/etc/letsencrypt/live/$DOMAIN" ]; then
        echo -e "${GREEN}✔ SSL certificate for $DOMAIN already exists.${NC}"
    else
