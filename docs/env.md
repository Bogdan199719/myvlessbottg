...
`ADMIN_TELEGRAM_ID` принимает один ID или несколько положительных числовых Telegram ID через запятую, например `123456789,987654321`. Каждый указанный пользователь получает административное меню и право обрабатывать администраторские действия; уведомления отправляются всем указанным администраторам.
...
- Проект закрепляет `py3xui==0.7.0`: Bearer token и CSRF-login используются через штатный SDK API.
- Для старых панелей используется legacy API `/panel/api/inbounds/*`.
- Для актуальных панелей приоритетно используется `/panel/api/clients/*`, а legacy API остаётся fallback.
- Hysteria/Hysteria2 обслуживаются через raw JSON API, пока SDK не моделирует поле клиента `auth`.
