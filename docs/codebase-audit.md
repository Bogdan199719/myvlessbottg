...
- Выручка фиксации всегда пересчитывается по журналу оплаченных транзакций и не может быть подменена ручным вводом при создании или редактировании периода.
...
- Расчёт использует точность до копейки и правило: налог Влада `9%` вычитается из всей выручки, затем вычитается полная стоимость серверов, остаток делится Богдану `40%` и Владу `60%`.
- Выплаченную фиксацию нельзя отменить; история показывает расхождение с текущим платёжным журналом.
- Удаление пользовательского профиля сохраняет `transactions`, чтобы не переписывать историческую выручку и закрытые периоды.

### Проверка

- Добавлен `scripts/check_profit_accounting.py`: временная SQLite проверяет выручку по локальному дню, первое paid-событие, пересечения `profit_distributions`, неизменяемость выплаченных фиксаций и сохранение платёжного журнала при удалении профиля.
