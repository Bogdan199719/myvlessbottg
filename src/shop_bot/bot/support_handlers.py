    ...
    if active_keys:
        ...
        for key, expiry_dt in active_keys:
            ...
            summary_parts.append(
                f"- <code>{key['key_email']}</code> (до {expiry} на хосте {key['host_name']})"
            )
    else:
...
def get_support_router() -> Router:
    ...
    async def handle_start(message: types.Message, bot: Bot):
        ...
        if not thread_id:
            ...
            try:
                ...
                logger.info(
                    f"Created new support thread {thread_id} for user {user_id}"
                )
                ...
                if isinstance(e, TelegramMigrateToChat):
                    ...
                    if new_id is None:
                        logger.error(
                            "Support group migration exception did not include a target chat ID."
                        )
                        await message.answer(
                            "Не удалось создать тикет в поддержке. Пожалуйста, попробуйте позже."
                        )
                        return
                    logger.info(
                        f"Support group migrated to supergroup. Updating ID to {new_id}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Created new support thread {thread_id} for user {user_id} after group migration"
                        )
                    ...
                    logger.error(
                        f"Failed to create support thread for user {user_id}: {e}",
                        exc_info=True,
                    )
                    await message.answer(
                        "Не удалось создать тикет в поддержке. Пожалуйста, попробуйте позже."
    ...
    async def from_user_to_admin(message: types.Message, bot: Bot):
        ...
        async def AttemptCopy(target_thread_id):
            ...
            try:
                ...
                if target_thread_id is not None and real_thread_id != target_thread_id:
                    logger.warning(
                        f"Probe message fell back to General/Other (Expected {target_thread_id}, got {real_thread_id}). Thread is effectively dead."
                    )
                    return False
                ...
                if isinstance(e, TelegramMigrateToChat):
                    ...
                    if new_id is None:
                        logger.error(
                            "Support group migration during copy did not include a target chat ID."
                        )
                        return None
                    logger.info(
                        f"Support group migrated to {new_id} during copy. Updating..."
                    )
                    database.update_setting("support_group_id", str(new_id))
                    SUPPORT_GROUP_ID = new_id
                ...
                if isinstance(e, TelegramBadRequest) and (
                    "thread not found" in error_msg or "topic_deleted" in error_msg
                ):
                    logger.warning(
                        f"Thread {target_thread_id} explicitly not found/deleted. Recreating..."
                    )
        ...
        if not success:
            await message.answer(
                "⚠️ Не удалось отправить сообщение в поддержку. Пожалуйста, попробуйте еще раз позже."
            )
            logger.error(
                f"Failed to copy message from user {user_id} to thread {thread_id}"
            )
