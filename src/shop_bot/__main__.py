    ...
    async def start_services():
        ...
        def run_flask():
            try:
                ...
                logger.info(
                    "Starting production server (waitress) on http://0.0.0.0:1488"
                )
                ...
                flask_app.run(
                    host="0.0.0.0", port=1488, threaded=True, use_reloader=False
                )
        ...
        logger.info(
            "Flask server started in a background thread on http://0.0.0.0:1488"
        )
        ...
        try:
            ...
            if shop_result["status"] == "success":
                ...
                logger.warning(
                    f"⚠️ Telegram Bot auto-start skipped: {shop_result['message']}"
                )
            ...
            if support_result["status"] == "success":
                ...
                logger.warning(
                    f"⚠️ Support Bot auto-start skipped: {support_result['message']}"
                )
        ...
        try:
            ...
            if str(xtls_enabled).strip().lower() in {"1", "true", "yes", "on"}:
                ...
                try:
                    ...
                    if sync_results and isinstance(sync_results, dict):
                        for host_name, result in sync_results.items():
                            if isinstance(result, dict):
                                ...
                                if fixed > 0:
                                    logger.info(
                                        f"Startup XTLS sync for '{host_name}': {fixed} clients fixed. Status: {status}"
                                    )
                                elif status == "success":
                                    logger.debug(
                                        f"Startup XTLS sync for '{host_name}': no fixes needed."
                                    )
                                else:
                                    logger.warning(
                                        f"Startup XTLS sync for '{host_name}': status={status}"
                                    )
                    if total_fixed > 0:
                        logger.info(
                            f"Startup XTLS synchronization completed: {total_fixed} total clients fixed"
                        )
                    else:
                        logger.info(
                    ...
                    logger.error(
                        f"Error during startup XTLS synchronization: {e}", exc_info=True
                    )
            else:
                logger.debug("Startup XTLS sync disabled (xtls_sync_enabled=false).")
        except Exception as e:
            logger.error(
                f"Failed to read xtls_sync_enabled setting: {e}", exc_info=True
            )
